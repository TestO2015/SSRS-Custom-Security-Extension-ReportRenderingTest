...
namespace Saturn.SSRSSecurityExtension
{
    public class SSRSAuthentication : IAuthenticationExtension2
    {
        ...
        public void GetUserInfo(out IIdentity userIdentity, out IntPtr userId)
        {
            var claims = new[]
            {
                new Claim(ClaimTypes.Name, "CORP\\yosef.shellim"),
            };
            userIdentity = new ClaimsIdentity(claims, "CustomAuthenticationType");
            userId = IntPtr.Zero;
            return;

            //var claimsPrincipal2 = LogonUserInternal("Yosef.Shellim", "CORP", "Poffice34a.");
            //userIdentity = claimsPrincipal2.Identity;
            //userId = IntPtr.Zero;
            //return;
            //var claimsPrincipal = new ClaimsPrincipal();
            //claimsPrincipal.Claims = new ClaimsPrincipal.Append(ClaimTypes.Name, @"CORP\Yosef.Shellim");

            //userIdentity = null;
            //userId = IntPtr.Zero;

            //userIdentity = HttpContext.Current?.User.Identity ?? throw new NullReferenceException("Anonymous logon is not configured. userIdentity should not be null!");
            //return;

            //string token = SSRSTokenAuthenticator.Current.IdToken(HttpContext.Current.Request);
            //token = System.IO.File.ReadAllText(@"c:\temp\token.txt");
        }
        ...
        public void GetUserInfo(IRSRequestContext requestContext, out IIdentity userIdentity, out IntPtr userId)
        {
            ...
            //userIdentity = requestContext.User ?? new GenericIdentity("CORP\\yosef.shellim");
